...
def hs_doi_mat_khau(TaiKhoan, matkhau_cu, matkhau_moi):
    ThongBao = ""
    hs = db_session.query(HocSinh).filter(HocSinh.IDHocSinh == TaiKhoan).first()
    if not (bcrypt.check_password_hash(hs.MatKhau, matkhau_cu)):
        ThongBao = "Mật khẩu không khớp"
    else:
        encrypt_pw_new = bcrypt.generate_password_hash(matkhau_moi).decode("utf-8")
        hs.MatKhau = encrypt_pw_new
        db_session.flush()
        db_session.commit()
